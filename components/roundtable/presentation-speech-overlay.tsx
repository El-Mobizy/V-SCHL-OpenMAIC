    ...
    <div className="flex items-center gap-2" onClick={onExpand}>
      <div
        className={cn(
          ...
          bubble.role === 'user'
            ? 'bg-cyan-50/80 dark:bg-cyan-950/70 border-cyan-200/70 dark:border-cyan-800/60'
            : bubble.role === 'agent'
      ...
      {onPlayPause && (
        <div
          ...
          className={cn(
            ...
            'hover:shadow-lg hover:scale-[1.02] active:scale-[0.98]',
            bubble.role === 'user'
              ? 'bg-cyan-50/80 dark:bg-cyan-950/70 border-cyan-200/70 dark:border-cyan-800/60 hover:bg-cyan-100 dark:hover:bg-cyan-900/70'
              : bubble.role === 'agent'
  ...
  return (
    <div
      ...
      className={cn(
        ...
        bubble.role === 'user'
          ? 'bg-cyan-50/60 dark:bg-cyan-950/55 border-cyan-200/70 dark:border-cyan-800/60'
          : bubble.role === 'agent'
    ...
    >
      ...
      {bubble.role !== 'user' &&
        ...
        buttonState !== 'none' &&
        (() => {
          const barsColor = bubble.role === 'agent' ? '#3b82f6' : '#f59e0b';
          ...
          if (buttonState === 'play') {
            return (
              <div
                ...
                className="absolute right-2.5 bottom-2.5 z-20 p-1.5 rounded-full bg-white/40 dark:bg-gray-800/40 backdrop-blur-sm group-hover/bubble:bg-amber-100 dark:group-hover/bubble:bg-amber-900/50 transition-all duration-300 cursor-pointer"
              >
                <Play className="w-3.5 h-3.5 text-gray-400 dark:text-gray-500 group-hover/bubble:text-amber-600 dark:group-hover/bubble:text-amber-400 ml-0.5" />
              </div>
            );
          }
          ...
          if (buttonState === 'restart') {
            return (
              <div
                ...
                className="absolute right-2.5 bottom-2.5 z-20 p-1.5 rounded-full bg-white/40 dark:bg-gray-800/40 backdrop-blur-sm group-hover/bubble:bg-amber-100 dark:group-hover/bubble:bg-amber-900/50 transition-all duration-300 cursor-pointer"
              >
                <Repeat className="w-3.5 h-3.5 text-gray-400 dark:text-gray-500 group-hover/bubble:text-amber-600 dark:group-hover/bubble:text-amber-400" />
              </div>
            );
          }
          ...
          return (
            ...
            >
              {isPaused ? (
                <Play className="w-3.5 h-3.5 text-amber-500 dark:text-amber-400 group-hover/bubble:text-amber-600 dark:group-hover/bubble:text-amber-400 ml-0.5" />
              ) : (
                <>
